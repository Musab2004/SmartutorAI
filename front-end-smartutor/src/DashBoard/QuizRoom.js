...
import { Tabs, Tab,Button ,Row,Col,Modal,Container,Alert,ButtonGroup,Form} from 'react-bootstrap';
...
if (userData) {
  const quizRoomData = {
   ...
   userService.post('/api/quizroom/', quizRoomData)
    .then(response => {
...
if (topics){
  ...
  options = Array.isArray(topicses) ? topicses.map(topic => ({
    value: topic.pk,
...
<div className="mb-3">
  <label htmlFor="time-picker" className="form-label">Select Time</label>
  <div>
