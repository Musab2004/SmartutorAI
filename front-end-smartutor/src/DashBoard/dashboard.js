...
<Sidebar studyPlan_id={studyPlan.id}/>
    ...
    <div className="App">
      {showChat && (
        <div style={{ position: 'fixed', bottom: '10px', right: '10px', width: '300px', height: '400px', backgroundColor: 'white', border: '1px solid black', borderRadius: '10px', padding: '10px', boxSizing: 'border-box', overflow: 'hidden' }}>
          ...
          <input type="text" onKeyDown={(e) => { if (e.key === 'Enter' && e.target.value.trim()) { sendMessage(e.target.value); e.target.value = ''; } }} placeholder="Type your message..." style={{ width: '100%', boxSizing: 'border-box' }} />
        </div>
      )}
     </div>
    {!showChat && (   <Button onClick={() => setShowChat(true)}  style={{ position: 'fixed',backgroundColor:'white',color:'black', bottom: '20px', right: '20px', fontSize: '16px' }}>
