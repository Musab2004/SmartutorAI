...
const fetchStudy_Plan = async () => {
    try { 
      const response = await userService.get(`/api/studyplans/${id}`);
...
useEffect(() => {
...
}, [studyPlan]); // This will run whenever studyPlan changes
  ...
  const NextStep  =async () => {
    ...
    const WeeklyTopics = [];
    const topicsPerWeek = Math.ceil(all_topics.length / studyPlan.duration);
    ...
    for (let i = 0; i < all_topics.length; i += topicsPerWeek) {
      ...
      const response = await userService.post('/api/weeklygoals/',postData);
        ...
        navigate('/finalstep', { state: { books: studyPlan } });
