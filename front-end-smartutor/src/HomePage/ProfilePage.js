...
const data = {
    ...
    if (Dates) {
      ...
      markedDates=updatedDates.map(dateString => new Date(dateString).toISOString().split('T')[0]);

 
    }
    const dateStrings = ['2024-3-8', '2024-3-9'];
    const markedDates4 = dateStrings.map(dateString => new Date(dateString).toISOString().split('T')[0]);
  ...
  <Modal.Body>
    ...
    {!isVerified && (
      <Form.Group>
        ...
        <Form.Control type="password" name="oldPassword" onChange={handleOldPasswordChange} />
  ...
  tileContent={({ date, view }) => {
    if (view === 'month' && markedDates.includes(date.toISOString().split('T')[0])) {
